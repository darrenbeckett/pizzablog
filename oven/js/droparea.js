(function( $ ){
    ...
    var m = {
        ...
        control: function(file, input, area){
            ...
            var types = (input.data('type') && input.data('type').split(/,/)) || [];
            
            //console.log( types+":"+tld+":"+file.type+":"+types.indexOf(file.type));

            // File type control
            if (input.data('type') && types.indexOf(tld)<0) {
                s.error({
                    'error':'Upload Error: Only specific files are allowed (' + input.data('type') + ')'
                }, input, area);
                return false;
            }
            ...
            if (file.size > (s.maxsize * 1048576)) {
                ...
                s.error({
                    'error':'Upload Error: Max upload size (' + s.maxsize + 'MB)'
                }, input, area);
                return false;
            }
        ...
        resize: function(file, input, area){
            ...
            reader.onloadend = function(e) { 
                ...
                if (data.length <= 6) {
                    s.error({
                        'error':'Upload Error: Unable to store file.'
                    }, input, area);
                    return 0;					
                } else {
    ...
    $.fn.droparea = function(o) {
        ...
        s = {
            ...
            'over'        : 'Drop Image Here',
            'nosupport'   : 'Your browser does not support this plugin.',
        ...
        this.each(function(){
            ...
            if(input.data('value') && input.data('value').length) {
                ...
                instructions.attr('id','ins_'+$(this).attr('id').replace('meat','')).addClass('instructions').html(s.instructions);
            }
            ...
            $(document).bind({
                ...
                dragenter: function (e) {
                    e.preventDefault();
                    instructions.removeClass('error').addClass('instructions over').html(s.over);
                },
                dragover: function (e) {
                    e.preventDefault();
                    instructions.removeClass('error').addClass('instructions over').html(s.over);
